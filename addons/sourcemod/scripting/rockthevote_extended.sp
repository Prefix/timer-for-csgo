...
AttemptRTV(client)
{
	...
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "RTV Not Allowed");
		return;
	}
		
	if (!CanMapChooserStartVote())
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "RTV Started");
		return;
	}
	
	if (GetClientCount(true) < GetConVarInt(g_Cvar_MinPlayers))
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Minimal Players Not Met");
		return;			
	}
	
	if (g_Voted[client])
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Already Voted", g_Votes, g_VotesNeeded);
		return;
	}	
	...
	CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "RTV Requested", name, g_Votes, g_VotesNeeded);
...
StartRTV()
{
	...
	if (EndOfMapVoteEnabled() && HasEndOfMapVoteFinished())
	{
		...
		{
			CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Changing Maps", map);
...
public Action:Command_ForceRTV(client, args)
{
	...
	CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Initiated Vote Map");
