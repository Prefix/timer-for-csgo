...
public Action:Command_SetNextmap(client, args)
{
	if (args < 1)
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] Usage: sm_setnextmap <map>");
	...
	if (!IsMapValid(map))
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Map was not found", map);
		return Plugin_Handled;
	}
...
public Action:Timer_StartMapVote(Handle:timer, Handle:data)
{
	...
	if (timePassed == 0 || !GetConVarBool(g_Cvar_HideTimer))
	{
		...
		switch(timerLocation)
		{
			...
			{
				CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", warningPhrase, warningTimeRemaining);
			}
...
{
	CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Initiated Vote Map");
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	if (IsVoteInProgress() || (g_NativeVotes && NativeVotes_IsVoteInProgress()))
	{
		...
		CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Cannot Start Vote", FAILURE_TIMER_LENGTH);
		new Handle:data;
...
//	Call_Finish();
	...
	CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Nextmap Voting Started");
}
						   ...
						   const item_info[][2])
{
	...
	if (strcmp(map, VOTE_EXTEND, false) == 0)
	{
		...
		CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Current Map Extended", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else if (strcmp(map, VOTE_DONTCHANGE, false) == 0)
	{
		...
		CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else
	{
		...
		CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Nextmap Voting Finished", map, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
						   ...
						   const item_info[][2])
{
	...
	if (GetConVarBool(g_Cvar_RunOff) && num_items > 1 && g_RunoffCount < GetConVarInt(g_Cvar_MaxRunOffs))
	{
		...
		if (highest_votes == item_info[1][VOTEINFO_ITEM_VOTES])
		{
			...
			CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Tie Vote", GetArraySize(mapList));
		...
		else if (highest_votes < required_votes)
		{
			...
			CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Revote Is Needed", required_percent);
			SetupWarningTimer(WarningType_Revote, MapChange:g_ChangeTime, mapList);
			return;
		}
	}
