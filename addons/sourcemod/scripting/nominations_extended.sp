...
public Action:Command_Addmap(client, args)
{
	if (args < 1)
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] Usage: sm_nominate_addmap <mapname>");
	...
	{
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Map was not found", mapname);
		return Plugin_Handled;		
	}
	...
	if (result > Nominate_Replaced)
	{
		...
		CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Map Already In Vote", mapname);
	...
	CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Map Inserted", mapname);
...
public Action:Command_Nominate(client, args)
{
	...
	if ((status & MAPSTATUS_DISABLED) == MAPSTATUS_DISABLED)
	{
		...
		{
			CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Can't Nominate Current Map");
		}
		
		if ((status & MAPSTATUS_EXCLUDE_PREVIOUS) == MAPSTATUS_EXCLUDE_PREVIOUS)
		{
			CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Map in Exclude List");
		}
		
		if ((status & MAPSTATUS_EXCLUDE_NOMINATED) == MAPSTATUS_EXCLUDE_NOMINATED)
		{
			CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Map Already Nominated");
		}
	...
	CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Map Nominated", name, mapname);
...
public Handler_MapSelectMenu(Handle:menu, MenuAction:action, param1, param2)
{
	switch (action)
	{
		case MenuAction_Select:
		{
			...
			if (result == Nominate_AlreadyInVote)
			{
				CPrintToChat(param1, "[{GREEN}TIMER{NORMAL}] %t", "Map Already Nominated");
				return 0;
			}
			else if (result == Nominate_VoteFull)
			{
				CPrintToChat(param1, "[{GREEN}TIMER{NORMAL}] %t", "Max Nominations");
				return 0;
			}
			...
			if (result == Nominate_Replaced)
			{
				CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Map Nomination Changed", name, map);
				return 0;	
			}
			
			CPrintToChatAll("[{GREEN}TIMER{NORMAL}] %t", "Map Nominated", name, map);
...
stock bool:IsNominateAllowed(client)
{
	...
	switch(result)
	{
		...
		{
			CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Nextmap Voting Started");
			return false;
		}
		...
		{
			CPrintToChat(client, "[{GREEN}TIMER{NORMAL}] %t", "Max Nominations");
			return false;
		}
	}
